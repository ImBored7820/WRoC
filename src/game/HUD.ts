...
export function setClassHover(mx: number, my: number, canvas: HTMLCanvasElement) {
...
function getClassCardRects(canvas: HTMLCanvasElement) {
    const cardW = 220;
    const cardH = 140;
    const gap = 20;
...
export function getClassFromClick(mx: number, my: number, canvas: HTMLCanvasElement): string | null {
...
export function drawClassSelect(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) {
...
export function drawHUD(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, player: Player, killCount: number) {
    const barWidth = 200;
    const padding = 20;
    ...
    // Kill counter badge — top left (now below controls panel)
    const badgeW = 110;
    const badgeH = 28;
...
function drawAbilityCooldownRing(
    ctx: CanvasRenderingContext2D, cx: number, cy: number, radius: number, player: Player
) {
...
export function drawControlsPanel(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) {
    if (!canvas) return;
    const x = 16;
    const y = 16;
    const width = 160;
    ...
    // Background: rgba(0, 0, 0, 0.6), 8px border-radius
    ...
    // Title: "CONTROLS" in bold 11px monospace, color #aaaaaa
    ...
    for (const row of rows) {
        // key label in #f0c040 (gold), description text in #cccccc
        // Key label left-aligned in a fixed 60px column, description right of it.
